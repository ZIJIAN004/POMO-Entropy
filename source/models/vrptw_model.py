"""VRPTW Model for POMO — 多车时间窗，无容量约束。"""
...
class VRPTWModel(nn.Module):
    def __init__(self, embedding_dim, encoder_layer_num, head_num, qkv_dim,
        ...
        # Encoder：depot 2D，customer 4D (x, y, tw_start, tw_end)
        ...
        # Decoder：context = last_node_embedding + current_time
        ...
        self.tw_end_max    = None   # 用于归一化 current_time

    def pre_forward(self, reset_state):
        node_feat = torch.cat((
            reset_state.node_xy,
            reset_state.tw_start[:, 1:, None],   # 客户 tw_start
            reset_state.tw_end[:, 1:, None],      # 客户 tw_end
        ), dim=2)   # (batch, problem, 4)

        x = torch.cat((self.embedding_depot(reset_state.depot_xy),
                        self.embedding_node(node_feat)), dim=1)
        ...
        self.tw_end_max      = reset_state.tw_end[:, 1:].max(dim=1).values   # (batch,)
    ...
    def forward(self, state):
        ...
        if state.selected_count == 0:
            ...
            norm_time = state.current_time / (self.tw_end_max[:, None] + 1e-6)
            q_input   = torch.cat((enc_last, norm_time[:, :, None]), dim=2)
