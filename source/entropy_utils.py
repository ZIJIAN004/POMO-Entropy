"""
Mode A entropy weighting — per-instance multivariate OLS on hand-crafted features.

Pipeline (identical to Mode B's downstream, just different residual):
...
`extract_mode_a_features_*` helpers in source.baseline).  TSP uses 2 features
...
from source.baseline import batched_per_instance_ols, compute_mode_b_weights
...
def compute_entropy_weights(entropy, features, n_feasible, advantage,
    ...
    return compute_mode_b_weights(residual, n_feasible, advantage, valid_mask, gamma=gamma)
